...
\title{Six-dimensional extensions of the five-dimensional kissing\\
configurations $Q_5$ and $R_5$: a proof of a conjecture of Cohn and Rajagopal\\[0.6ex]
{\large with rigidity and maximality certificates for $D_5$, $L_5$, $Q_5$, $R_5$}}
\author{Stephan Mitterer\thanks{%
...
author is responsible for this publication, the repository and this text;
the mathematics was developed by the systems named above and verified as
...
\begin{abstract}
We prove Conjecture~3.1 of Cohn and Rajagopal: no six-dimensional
kissing configuration with Sz\"oll\H{o}si's $Q_5$ or Cohn--Rajagopal's
$R_5$ as a cross section contains $72$ or more points; in fact at most
$70$. The proof combines a reduction lemma (any such cross section is
central and the configuration splits into two independent layers), a
norm-coupling inequality showing each layer projects to a spherical code
in $S^4$ with inner products at most $1/5$, the sharp
linear-programming bound $A(5,1/5)=16$ with its equality analysis
\`a la Delsarte--Goethals--Seidel~\cite{DGS1977}, and the exact
...
cross sections and conjecture that $64$ is optimal.
As supporting results, obtained by exact computation, we show that the
four known $40$-point kissing configurations in $\R^5$ --- the $D_5$
configuration, Leech's $L_5$, $Q_5$, and $R_5$ --- are infinitesimally
jammed, hence jammed (for $Q_5$ this answers a question of
Sz\"oll\H{o}si; for $D_5$, $L_5$ it reproduces results of Cohn, Jiao,
Kumar, and Torquato), and that no $41$st point can be adjoined to any of
the four; the second fact is an input to the main argument.
All certificates are exact data (rational, or in $\Q(\sqrt3)$ for the
$64$-point extensions), verified by independent computations, and
accompany the paper.
\end{abstract}
...
\cite{CR2024} subsequently found a fourth, $R_5$.
...
Cohn and Rajagopal \cite[\S3]{CR2024} extend $D_5$ and $L_5$ to six
dimensions by placing $16$-element sets of pairwise compatible deep
holes in two layers above and below the cross section; this yields
$72$-point six-dimensional kissing configurations, among them those of
$E_6$ and of Leech. For $Q_5$ and $R_5$ the same construction breaks
down --- no $16$ deep holes are pairwise compatible
(Lemma~\ref{lem:holeclique}) --- and they conjectured
\cite[Conjecture~3.1]{CR2024} that no six-dimensional kissing
configuration with $Q_5$ or $R_5$ as a cross section contains $72$ or
more points. Our main result proves this with the stronger bound $70$.
...
Two facts about the known configurations accompany the main result.
The first is an input to its proof:
...
The second is not used in the proof but is of independent interest:
...
Sz\"oll\H{o}si explicitly raised the rigidity of $Q_5$ as a question;
the question matters because a flexible optimal configuration would open
a deform-and-augment route towards $\kappa(5)\ge 41$.

We emphasize what is \emph{not} claimed: Theorems~\ref{thm:no41} and
\ref{thm:jam} are statements about the known configurations and imply
...
Write $X=\{x_1,\dots,x_{40}\}$. Then $\sum_i x_i=0$ exactly and $X$ spans
$\R^5$, so the origin is interior
...
$D_5,L_5$). In the normalization of \cite[\S3]{CR2024} the holes lie at
...
$H\cap S$ with center $cn$ and squared radius $2-c^2$; hence
...
\begin{proof}
This is a case of Levenshtein's bound \cite{Lev1979}; we include the
explicit certificate since its equality analysis is used below.
...
pairwise inner products lie in $\{1/5,-3/5\}$. This code attains
Levenshtein's bound \cite{Lev1979} and is the unique $(5,16,1/5)$
spherical code up to isometry \cite[Appendix~A]{CK2007}. This is exactly half of
...
exact hole-clique number $12$ and by a multistart penalty minimization,
$200$ restarts per case, script
\texttt{experiments/conjecture31\_stageA.py}, which reaches $12$ points to
machine precision and finds no $13$-point cap set; this probe is not part
of the verified claims. Closing $12$ versus $15$ requires more than the LP
bound, by Remark~\ref{rem:sharp}.)
\end{conjecture}

\section{Jamming}\label{sec:jam}

We record the rigidity of the four configurations. This is not used
above; it answers Sz\"oll\H{o}si's question and closes the
deform-and-augment route mentioned in the introduction.
...
certificates; ranks are recomputed exactly by two independent
implementations and clique numbers by two algorithms. Three of the
computational claims are checked by two independent computations: the
...
\texttt{github.com/smitterer/kissingnumbers}, release v1.1) contains all
...
We thank Henry Cohn for suggestions on the presentation of this note and
for pointing out the reference \cite{Lev1979}.
...
\bibitem{Lev1979} V.I.~Levenshtein, On bounds for packings in
$n$-dimensional Euclidean space, \emph{Dokl.\ Akad.\ Nauk SSSR}
\textbf{245} (1979), no.~6, 1299--1303 (Russian); English translation:
\emph{Soviet Math.\ Dokl.} \textbf{20} (1979), no.~2, 417--421.
\bibitem{CK2007} H.~Cohn, A.~Kumar, Universally optimal distribution of
points on spheres, \emph{J.\ Amer.\ Math.\ Soc.} \textbf{20} (2007),
no.~1, 99--148.
\end{thebibliography}
